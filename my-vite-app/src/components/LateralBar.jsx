...
export default function LateralBar({ open, onClose, title, children, side, noOverlap }) {
    return (
        <div className={open === true ? `lateralbar-container-${side && side === 'left' ? 'left' : 'right'}` : 'page-closed'} style={

            noOverlap && !window.matchMedia('(max-width: 1300px)') ? { position: 'relative' } : null
        }>
