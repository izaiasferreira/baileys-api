...
const { AuthKeyModel } = require("./auth.model");

async function useMongoAuthState(sessionID, saveOnlyCreds = false) {
  ...
  async function writeData(data, key) {
    ...
    await AuthKeyModel.findOneAndUpdate(
      { botId: sessionID, keyId: key },
  ...
  async function readData(key) {
    try {
      ...
      if (saveOnlyCreds && key !== "creds") {
        ...
        const authKey = await AuthKeyModel.findOne({
          botId: sessionID,
          keyId: key,
        });
  ...
  async function removeData(key) {
    try {
      if (saveOnlyCreds && key !== "creds") {
        ...
        await AuthKeyModel.deleteOne({ botId: sessionID, keyId: key });
      }
    } catch (error) {
